...
This document describes the structural-only GWM restored on 2026-08-02. The
current code uses normalized dot-product retrieval. It retains the context
Transformer, transition Transformer, shared forward/inverse relation
parameterization, full-entity cross-entropy, and masked world-state
reconstruction. The exact restored combination has not yet been retrained, so
historical results below are labeled by their actual variants.
...
The decoder receives two target-side tokens:

`[z_r + role_relation, next_state_token]`.

A causal mask prevents the relation token from reading the next-state token,
while the next-state token can read the relation token. Both tokens cross-attend
to `M_h`. After two decoder layers, the next-state output is linearly projected
and normalized:

`q_(h,r) = normalize(W_next Decoder(z_r, next_token; M_h)[next])`.
...
Candidates are ranked with temperature-scaled cosine similarity:

`s(h,r,t) = q_(h,r)^T v_t / tau`.
...
These are not results for the exact restored code and should not be presented as
...
- Earlier multi-particle and multi-positive variants did not improve the main
...
1. **Single deterministic next state.** One vector must rank every valid tail of
   a 1-N or N-N query, encouraging a compromise representation.
...
1. **State/objective mismatch for multi-answer relations.** This is the strongest
   bottleneck because it appears consistently in arity metrics on both datasets.
